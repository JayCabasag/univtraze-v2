...
function SignUp({ navigation, route }) {
      ...
      await genericPostRequest('/auth/signup', payload)
        ...
        .catch((error) => {
          ...
          if (error.response.status === 409) {
            Alert.alert('User already exists', 'Please use a different email', [{ text: 'OK' }]);
  ...
  const hasTermsAndConditionsError =
      ...
      <KeyboardAvoidingView style={styles.container} behavior="height">
        <View style={styles.inputContainer}>
          ...
          {hasConfirmPasswordError && (
            <Text style={styles.errorLabel}>*{formik.errors.confirmPassword}</Text>
          )}
          ...
          <View style={{ display: 'flex', flexDirection: 'row', marginVertical: 15 }}>
            <Checkbox
              ...
              onValueChange={() =>
                handleInputTextChange(!formik.values.termsAndConditions, 'termsAndConditions')
              }
              style={
          ...
          {hasTermsAndConditionsError && (
            <Text style={styles.errorLabel}>*{formik.errors.termsAndConditions}</Text>
          )}
